<x-app-layout>
  ...
  <div class="bg-white">
        ...
        <div class="border-t border-gray-200">
          <dl>
            ...
            <div class="col-span-3 flex justify-center">
              ...
              <button id="{{ $character->id }}" onclick="likeCharacter(this)"
    ...
    <h2 class="text-xl font-extrabold ml-2 mt-5">Comics</h2>
                    ...
                    <td class="px-2 py-4 whitespace-nowrap">
                      <div class="flex items-center">
                        <div class="flex-shrink-0 h-10 w-10">
                          <img class="h-10 w-10"
                            src="{{ str_replace('portrait_uncanny', 'standard_small', $comic->thumbnail) }}"
                            alt="{{ $comic->title }}">
                        </div>
                        <div class="ml-4">
  ...
  <script>
    ...
    async function likeCharacter(heart) {
      ...
      (async function() {
        ...
        if (response.status == 200) {
          ...
          for (let i = 0; i < hearts.length; i++) {
            if (data.comics.includes(parseInt(hearts[i].id))) {
              hearts[i].classList.remove('text-gray-400', 'hover:text-red-500');
              hearts[i].classList.add('hover:text-gray-400', 'text-red-500');
            }
          }
          if (data.characters.includes({{ $character->id }})) {
            const heart = document.getElementById({{ $character->id }});
            heart.classList.remove('text-gray-400', 'hover:text-red-500');
            heart.classList.add('text-red-500', 'hover:text-gray-400');
            heart.innerHTML = heart.innerHTML.replace('Add to favorites', 'Remove from favorites');
          }
